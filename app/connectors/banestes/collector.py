...
Página de publicações legais / editais de alienação (HTML índice) → links de
editais PDF. PDFs extraídos com pdfplumber. Banco capixaba — default state ES.
"""
import re
...
from app.connectors.banestes.normalizer import BanestesNormalizer
from app.connectors.banestes.parser import BanestesParser
...
class BanestesConnector(BankConnector):
    bank_code = "banestes"

    def __init__(self, uf: str | None = None):
        self.uf = uf
        self.parser = BanestesParser()
        self.normalizer = BanestesNormalizer()

    def discover_sources(self) -> list[str]:
        sources = [BANESTES_INDEX_URL]
        index_bytes = self.fetch_raw(BANESTES_INDEX_URL)
        sources.extend(self._extract_pdf_links(index_bytes, BANESTES_INDEX_URL))
        return sources
    ...
    def fetch_raw(self, source_url: str) -> bytes:
        try:
            ...
            logger.error("banestes.fetch_failed", url=source_url, error=str(exc))
            return b""

        if content[:4] != b"%PDF" and content[:5].lower() in (b"<html", b"<!doc"):
            head = content[:512].lower()
            if b"captcha" in head or b"challenge" in head:
                logger.warning("banestes.fetch_got_challenge", url=source_url)
                return b""
        logger.info("banestes.fetch_ok", url=source_url, size=len(content))
        return content
