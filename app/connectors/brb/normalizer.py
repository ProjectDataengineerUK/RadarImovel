...
class BRBNormalizer:
    ...
    def normalize(self, raw: RawProperty) -> dict:
        ...
        try:
            ...
            state = (clean_text(d.get("state")) or "DF").upper()[:2]
            modality = "Venda direta"
            if raw.source_name == "brb_resale":
                modality = "Venda direta (Resale)"
            ...
            logger.error("brb.normalizer.failed", external_code=raw.external_code, error=str(exc))
            raise
